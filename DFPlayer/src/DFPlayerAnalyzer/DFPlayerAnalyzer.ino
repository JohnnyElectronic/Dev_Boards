...
Driver library (slightly modified version): https://github.com/Makuna/DFMiniMp3
...
Setup of USB and/or SD card (minimum):
...
#define DEBUG_DFPLAYER_COMMUNICATION    // Provides detailed communication status
#include "DFMiniMp3.h"
  ...
  protected:
    void doRun() {
      ...
      if (sdAvailable) {
        ...
        busyWait(2000);
        int track = player.getCurrentTrack();
        while ((cntPlayFinishedCallback == 0) && (loopCnt < 20)) {
          busyWait(1000);
          loopCnt++;
        }
        ...
        if (loopCnt >= 20) {
            Serial.println(F("Callback timeout waiting on SD track end!"));
        }
...
void setup() {
  ...
  //Test* allTests[] {&t0, &t1, &t2, &t3, &t4, &t5, &t6, &t7};
   ...
   Test* allTests[] {&t0, &t1, &t2, &t3, &t4, &t5};
